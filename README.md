...
> 
- MacOS 에서 셋팅 (`완료`)
- WSL2(우분투)에서 셋팅 (`완료`)  <- 현재 readme 가이드  
- CentOS에서 셋팅(`진행예정`)
...
>💡 nextJS 설치했을때 react 18 이 설치 되어있다면 따로 진행 안해도 됨
> react 18버전 설치, 2022년 3월 nextJS 설치 기준으로는 react가 17버전이라 
> react18 rc를 설치했음
...
>📢 eslint 를 현재 mac에서 eslint,prettier 셋팅이 매끄럽지 않았는데 wsl2 에서 다시 셋팅해보았음 
> 적용은 되는데 옵션으로 줄수 있는게 너무 많음
